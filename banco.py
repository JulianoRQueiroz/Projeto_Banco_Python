...
def efetuar_deposito() -> None:
    if len(contas) > 0:
        ...
        if conta:
            valor: float = float(input('Informe um valor de deposito: '))
        ...
        print('Ainda não existem contar cadastradas!') 
    sleep(2)
    menu()
    ...
    if len(contas) > 0:
        ...
        if conta_o:
            ...
            if conta_d:
                ...
                conta_o.trasnferir(conta_d, valor)
            else: 
                print(f'A conta de destino con {numero_d} não foi encotrado')
        else:
